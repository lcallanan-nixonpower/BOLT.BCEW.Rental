...
namespace BOLT.BCEW.Rental.CostSheetChildren
{
    public class RentalGenerators_CostPriceCalculations : IPlugin
    {
        /// <summary>
        /// A plugin that calculates the Cost and Price fields for the Rental Generator entity. BCEW version
        /// </summary>
        /// <remarks>
        ...
        public void Execute(IServiceProvider serviceProvider)
        {
                ...
                context.InputParameters["Target"] is Entity)
            {
                ...
                try
                {
                    ...
                    if (!rerent) // Price basis - inputs represent price to customer
                    {
                        decimal daily_price_value = daily_input_value * shift_multipliers[shift_Value];
                        decimal weekly_price_value = weekly_input_value * shift_multipliers[shift_Value];
                        decimal monthly_price_value = monthly_input_value * shift_multipliers[shift_Value];

                        decimal daily_cost_value = daily_price_value * (1 - margin_Percent / 100);
                        decimal weekly_cost_value = weekly_price_value * (1 - margin_Percent / 100);
                        decimal monthly_cost_value = monthly_price_value * (1 - margin_Percent / 100);

                        setvalues(daily_cost_value, weekly_cost_value, monthly_cost_value, daily_price_value, weekly_price_value, monthly_price_value);
                    }
                    else // Cost basis - inputs represent costs to company
                    {
